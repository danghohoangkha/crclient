...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      //lay lich su choi
      let dataHistoryMatch = await api.get('/api/gethistorymatch/'+localStorage.getItem("id"));
      console.log(dataHistoryMatch);
      setHistoryMatch(dataHistoryMatch.data);
    }
    fetchData();
  }, [socket])
//Logout
  ...
  return (
    <>
      <Grid xs={12} >
        ...
        <Grid container xs={12} style={{top: 110, position: "fixed", height: "550px" }}>
            <Grid xs={9} style={{}}>
                <History historyMatch={historyMatch} />
            </Grid>
            <Grid xs={3}>
