...
function AskInvited({handleCloseAskInvited, onAcceptInvited, showAskInvited, invitedRoomInfor}){
  const classes = useStyles();
    return(
      <>
      <Modal show={showAskInvited} onHide={handleCloseAskInvited} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Body>
            <div style={{fontSize: "20px"}}>
                 {invitedRoomInfor? `${invitedRoomInfor.senderName} mời bạn vào phòng ${invitedRoomInfor.roomName}`: '' }
            </div>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCloseAskInvited} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={onAcceptInvited} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
