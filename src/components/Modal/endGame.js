...
function EndGame({endGame, handleContinue, handleCancel, messageMatch}){
  const classes = useStyles();
    return(
      <>
      <Modal show={endGame} onHide={handleCancel} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Body>
            <div style={{fontSize: "20px"}}>
                 <p>{messageMatch}</p>
                 <p>Bạn có muốn tiếp tục chơi?</p>
            </div>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCancel} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={handleContinue} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
