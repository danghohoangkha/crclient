...
function RepDraw({isDraw, onHandleDraw, handleCancelDraw}){
  const classes = useStyles();
    return(
      <>
      <Modal show={isDraw} onHide={handleCancelDraw} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Body>
            <div style={{fontSize: "20px"}}>
                 Bạn có chấp nhận lời cầu hoà không?
            </div>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCancelDraw} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={onHandleDraw} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
