...
function ValidLose({handleLose, validLose, handleCloseValidLose}){
  const classes = useStyles();
    return(
      <>
      <Modal show={validLose} onHide={handleCloseValidLose} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCloseValidLose} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={handleLose} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
