...
function AskLogout({handleCloseAskLogout, showAskLogout, onLogout}){
  const classes = useStyles();
    return(
      <>
      <Modal show={showAskLogout} onHide={handleCloseAskLogout} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCloseAskLogout} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={onLogout} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
