...
function AskLeaveRoom({handleCloseAskLeaveRoom, leaveRoom, handleLeaveRoom}){
  const classes = useStyles();
    return(
      <>
      <Modal show={leaveRoom} onHide={handleCloseAskLeaveRoom} animation={false} centered>
        <Paper container className={classes.paper}>
          ...
          <Modal.Body>
            <div style={{fontSize: "20px"}}>
                 Bạn có chắc chắn muốn thoát khỏi phòng chơi?
            </div>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCloseAskLeaveRoom} className={classes.button}>
              Huỷ bỏ
            </Button>
            <Button variant="primary" onClick={handleLeaveRoom} className={classes.button}>
              Đồng ý
            </Button>
          </Modal.Footer>
          </div>
